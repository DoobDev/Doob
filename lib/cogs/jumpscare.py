...
class Jumpscare(Cog):
    ...
    async def jumpscare_command(self, ctx, *, target: Member):
        ...
        if jumpscare == 1:
            ...
            embed = Embed(title="boo", description=f"{ctx.author.mention} fnaf jumpscare", colour=Colour.red())
            embed.set_footer(text=f"{target.display_name} boo", icon_url=target.avatar_url)
            embed.set_image(url = "https://i.pinimg.com/originals/ac/fd/40/acfd400be3e5a65503464e395e75947e.gif")
            await ctx.send(f"{target.mention} lol")
            await ctx.send(embed=embed)
            ...
            embed = Embed(title="boo", description=f"{ctx.author.mention} fnaf jumpscare", colour=Colour.red())
            embed.set_footer(text=f"{target.display_name} boo", icon_url=target.avatar_url)
            embed.set_image(url = "https://static.wikia.nocookie.net/fnafapedia/images/e/e4/Fnaf4_jumpscare_chicaindoorway.gif/revision/latest/top-crop/width/220/height/220?cb=20151031010318")
            await ctx.send(f"{target.mention} lol")
            await ctx.send(embed=embed)
            ...
            embed = Embed(title="boo", description=f"{ctx.author.mention} fnaf jumpscare", colour=Colour.red())
            embed.set_footer(text=f"{target.display_name} boo", icon_url=target.avatar_url)
            embed.set_image(url = "https://thumbs.gfycat.com/PertinentSevereKrill-max-1mb.gif")
            await ctx.send(f"{target.mention} lol")
            await ctx.send(embed=embed)
            ...
            embed = Embed(title="boo", description=f"{ctx.author.mention} fnaf jumpscare", colour=Colour.red())
            embed.set_footer(text=f"{target.display_name} boo", icon_url=target.avatar_url)
            embed.set_image(url = "https://thumbs.gfycat.com/ShamelessVainCrossbill-max-1mb.gif")
            await ctx.send(f"{target.mention} lol")
            await ctx.send(embed=embed)
