...
class gamestats(Cog):
    ...
    @command(
        name="owstats",
        aliases=["overwatch", "owprofile", "ow"],
        brief="Gets your Overwatch stats.",
    )
    ...
    async def overwatch_stats(self, ctx, target: Optional[Member]):
        ...
        async with request("GET", URL) as response:
            if response.status == 200:
                ...
                data = (await response.json())["competitiveStats"]["careerStats"][
                    "allHeroes"
                ]

                embed = Embed(
                    title=f"{target.display_name}'s Overwatch Stats!",
                    description="Competitive Stats",
                    colour=ctx.author.colour,
                )
                # All of the stats from the API, all into the fields.
                fields = [
                    (
                        "Below are per 10 min average stats.",
                        "-----------------------------------------",
                        False,
                    ),
                    (
                        "Eliminations",
                        f"{data['average']['eliminationsAvgPer10Min']}",
                        True,
                    ),
                    (
                        "All Damage Done",
                        f"{data['average']['allDamageDoneAvgPer10Min']}",
                        True,
                    ),
                    (
                        "Final Blows",
                        f"{data['average']['finalBlowsAvgPer10Min']}",
                        True,
                    ),
                    ("Solo Kills", f"{data['average']['soloKillsAvgPer10Min']}", True),
                    (
                        "Time spent on fire",
                        f"{data['average']['timeSpentOnFireAvgPer10Min']}",
                        True,
                    ),
                    (
                        "Healing done",
                        f"{data['average']['healingDoneAvgPer10Min']}",
                        True,
                    ),
                    ("Deaths", f"{data['average']['deathsAvgPer10Min']}", True),
                    (
                        "Hero Damage Done",
                        f"{data['average']['heroDamageDoneAvgPer10Min']}",
                        True,
                    ),
                    ...
                    (
                        "All Damage Done",
                        f"{data['best']['allDamageDoneMostInGame']}",
                        True,
                    ),
                    ...
                    (
                        "Time spent on fire",
                        f"{data['best']['timeSpentOnFireMostInGame']}",
                        True,
                    ),
    ...
    @command(
        name="setowusername",
        aliases=["setplatform", "sowp", "sowu", "setusername"],
        brief="Sets your Overwatch username+platform.",
    )
    @cooldown(1, 5, BucketType.user)
    async def set_overwatch_profile(
        self,
        ctx,
